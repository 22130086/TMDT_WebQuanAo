...
export default function FactoryProfile() {
    ...
    useEffect(() => {
        ...
        Promise.all([
            ...
            http.get(`/reviews/factories/${id}/products?page=0&size=5`),
            ...
            if (rRes.data?.data) {
                ...
                setTotalProductReviews(rRes.data.data.totalElements || 0);
            }
    ...
    const loadMoreReviews = async (p: number) => {
        try {
            const res = await http.get(`/reviews/factories/${id}/products?page=${p}&size=5`);
            if (res.data?.data) {
    ...
    // ✅ Dùng ký tự sao thật thay vì emoji repeat (hiển thị đẹp hơn, không bị lỗi khi rating = 0)
            ...
            <div style={{ maxWidth: 1200, margin: "0 auto", padding: "24px 16px" }}>
                                        ...
                                        {images.map((img, i) => (
                                            <img key={i} src={getImageUrl(img)} alt=""
                                                 ...
                                                 style={{ width: 64, height: 64, objectFit: "cover", borderRadius: 8,
                                                     cursor: "pointer",
                                                     border: i === imageIdx ? "2px solid #2563eb" : "2px solid transparent" }} />
                                        ))}
                                    </div>
                                )}
                            </>
                        ) : (
                            <div style={{ width: "100%", height: 280, background: "#f1f5f9", borderRadius: 16,
                ...
                {factory.verifiedStatus === "APPROVED" ? "Đã xác minh" : "Chờ duyệt"}
                        ...
                        <div style={{ display: "flex", gap: 6, marginBottom: 12, alignItems: "center" }}>
                            {renderStars(factory.ratingAvg || 0)}
                        ...
                        <div style={{ display: "flex", gap: 32, marginTop: 16 }}>
                            <div style={{ textAlign: "center" }}>
                                <strong style={{ fontSize: 20 }}>{factory.totalProducts || 0}</strong>
                                <p style={{ color: "#64748b", fontSize: 13 }}>Sản phẩm</p>
                            </div>
                            <div style={{ textAlign: "center" }}>
                                <strong style={{ fontSize: 20 }}>{factory.totalOrders || 0}</strong>
                                <p style={{ color: "#64748b", fontSize: 13 }}>Đơn hàng</p>
                            </div>
                            <div style={{ textAlign: "center" }}>
                                <strong style={{ fontSize: 20 }}>{factory.totalRatings || 0}</strong>
                                <p style={{ color: "#64748b", fontSize: 13 }}>Đánh giá</p>
                            </div>
                        </div>
                    </div>
                </div>
                                ...
                                <Link key={p.id} to={`/products/${p.id}`} style={{ textDecoration: "none", color: "inherit" }}>
                                    <div style={{ background: "#fff", borderRadius: 12, overflow: "hidden", border: "1px solid #e2e8f0" }}>
                                        {p.imageUrls?.[0] ? (
                                            ...
                                            <div style={{ width: "100%", height: 180, background: "#f1f5f9",
                                                display: "flex", alignItems: "center", justifyContent: "center" }}>
                ...
                <section>
                    <h2 style={{ fontSize: 20, fontWeight: 700, marginBottom: 16 }}>
                        Đánh giá ({factory.totalRatings || 0})
                    </h2>
                    {reviews.length === 0 ? (
                            ...
                            {reviews.map((r: ReviewItem) => (
                                <div key={r.id} style={{ padding: 16, background: "#fff", borderRadius: 12, border: "1px solid #e2e8f0" }}>
                                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                                        <img
                                            src={r.customerAvatar ? getImageUrl(r.customerAvatar) : "https://i.pravatar.cc/40"}
                                            alt=""
                    ...
                    {totalReviewPages > 1 && (
                        <div style={{ display: "flex", justifyContent: "center", gap: 8, marginTop: 16 }}>
                            <button disabled={reviewPage === 0} onClick={() => loadMoreReviews(reviewPage - 1)}
                                    style={{ padding: "6px 14px", border: "1px solid #ddd", borderRadius: 6, cursor: "pointer" }}>←</button>
                            <span style={{ padding: "6px 0" }}>{reviewPage + 1}/{totalReviewPages}</span>
                            <button disabled={reviewPage >= totalReviewPages - 1} onClick={() => loadMoreReviews(reviewPage + 1)}
                                    style={{ padding: "6px 14px", border: "1px solid #ddd", borderRadius: 6, cursor: "pointer" }}>→</button>
                        </div>
                    )}
                </section>
