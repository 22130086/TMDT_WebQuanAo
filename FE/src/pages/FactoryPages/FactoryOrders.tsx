...
export default function FactoryOrders() {
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(`
      <style>
        @page { margin: 0; }
        body { font-family: system-ui, -apple-system, sans-serif; color: #111827; margin: 0; padding: 20px; display: flex; justify-content: center; }
                          ...
                          {detailLoading ? <div style={{ padding: 40, color: "#6b7280" }}>Đang tải hóa đơn...</div> : (
                            <div id={`invoice-content-${selected.id}`} style={{ width: "100%", maxWidth: "450px", background: "#fff", padding: "24px", borderRadius: "12px", boxShadow: "0 4px 20px rgba(0,0,0,0.06)", border: "1px solid #e2e8f0", position: "relative" }}>
                              <div className="invoice-header" style={{ textAlign: "center", borderBottom: "1px dashed #cbd5e1", paddingBottom: "16px", marginBottom: "20px" }}>
                                <h2 className="screen-only" style={{ margin: 0, fontSize: "20px", color: "#111827", textTransform: "uppercase", letterSpacing: "1px" }}>Chi tiết đơn hàng</h2>
                                <h2 className="print-only" style={{ display: "none", margin: 0, fontSize: "20px", color: "#111827", textTransform: "uppercase", letterSpacing: "1px" }}>Hóa đơn bán hàng</h2>
                              ...
                              {selected.items && selected.items.length > 0 && (
                                <div style={{ marginBottom: "24px", width: "100%" }}>
                                  <div style={{ fontWeight: 700, color: "#9ca3af", textTransform: "uppercase", fontSize: "11px", borderBottom: "1px solid #e5e7eb", paddingBottom: "6px", margin: "0 0 8px 0" }}>Danh sách sản phẩm</div>
                                  {selected.items.map((it, i) => (
                                    <div key={i} style={{ borderBottom: "1px dashed #f1f5f9", padding: "12px 0", fontSize: "13px" }}>
                                      <div style={{ color: "#111827", fontWeight: 600, marginBottom: "6px" }}>{it.productName || `SP #${it.id}`}</div>
                                      <div style={{ display: "flex", justifyContent: "space-between" }}>
                                        <div style={{ color: "#6b7280", fontSize: "12px" }}>{formatVND(it.unitPrice)} x <strong>{it.quantity}</strong></div>
                                        <div style={{ color: "#111827", fontWeight: 700, fontSize: "14px" }}>{formatVND(it.unitPrice * it.quantity)}</div>
                                      </div>
                                    </div>
                                  ))}
                                </div>
                              ...
                              <div className="summary" style={{ borderTop: "1px dashed #cbd5e1", paddingTop: "16px" }}>
                                ...
                                <div className="screen-only" style={{ textAlign: "right", marginTop: "8px", fontSize: "12px", color: selected.paymentStatus === "FULLY_PAID" ? "#10b981" : "#ef4444", fontWeight: 600 }}>
